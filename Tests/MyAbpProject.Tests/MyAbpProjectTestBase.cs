...
{
    public abstract class MyAbpProjectTestBase : AbpIntegratedTestBase<MyAbpProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyAbpProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyAbpProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyAbpProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyAbpProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyAbpProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyAbpProjectDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyAbpProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyAbpProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyAbpProjectDbContext, Task<T>> func)
        {
            T result;
